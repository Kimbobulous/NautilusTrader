...
```bash
PHASE_INFO=$(node "C:/dev/nautilustrader/.codex/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "C:/dev/nautilustrader/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "C:/dev/nautilustrader/.codex/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
