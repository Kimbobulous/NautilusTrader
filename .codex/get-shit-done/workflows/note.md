...
- **Global scope**: `C:/dev/nautilustrader/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/dev/nautilustrader/.codex/notes/`)
...
2. Glob `C:/dev/nautilustrader/.codex/notes/*.md` (if directory exists) — global notes
...
Global (C:/dev/nautilustrader/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/dev/nautilustrader/.codex/notes/` — works in any directory
