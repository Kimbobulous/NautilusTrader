...
```bash
INIT=$(node "C:/dev/nautilustrader/.codex/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "C:/dev/nautilustrader/.codex/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
